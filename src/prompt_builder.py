...
STYLING — The garment is worn UNTUCKED. The hem falls at its designed position as shown in REFERENCE B on this specific wearer. Never tuck the garment into pants, skirts, or any waistband/belt on the lower body. If the hem lands above the wearer's natural waist, show bare midriff between the hem and the lower-body garment — do NOT hide the hem under a waistband. Do NOT invent design elements (collars, sleeves, hems, closures) that are not visible in REFERENCE B.
...
GARMENT PROFILE — The target piece is a "{name}" (catalog ID {gid}, category {category}), labeled size {size}, intended cut: "{fit_style}". Its measured specifications are: {garment_line}. These numbers are for circumference/fit math only — the visual design (sleeve length, hem position, neckline, silhouette, all visible details) comes from REFERENCE B and must match exactly what that image shows.
...
def _describe(dim: str, delta: float, person_val, garment_val, units: str) -> str:
    ...
    if dim == "chest":
        ...
        if delta <= -10:
            body = (
                f"The garment is far too small at the hip — fabric is at its maximum stretch. Render the cloth "
                f"pulled taut across the hip with the silhouette of the body reading clearly through, side "
                f"seams under heavy load, and any pockets pulled wide open. Keep it plausible."
            )
        elif delta <= -6:
            body = (
                f"The hip is heavily strained. Render visible tension lines from the upper outer thigh, body "
                f"contour reading through the cloth, and pockets gaping under load."
            )
        elif delta <= -3:
            body = (
                f"The hip is clearly tight. Side seams under load with faint tension lines at the upper outer "
                f"thigh and the body contour starting to read through the fabric."
            )
        elif delta < -1:
            body = (
                f"The hip is snug. The fabric sits in close contact across the hip with no air gap, but with "
                f"no visible stretch lines."
            )
        elif delta <= 1:
            body = (
                f"The hip is true to size — the garment skims the hip cleanly with no strain and no excess."
            )
        elif delta <= 3:
            body = (
                f"The hip is relaxed. A small air gap separates the fabric from the hip with soft straight "
                f"drape."
            )
        elif delta <= 6:
            body = (
                f"The hip is loose. Soft vertical folds fall from the waistband, with the fabric clearly off "
                f"the hip."
            )
        else:
            body = (
                f"The hip is heavily oversized. Render deep vertical folds from the waistband, with a baggy, "
                f"tent-leaning silhouette around the hip and upper thigh."
            )

    elif dim == "thigh":
        if delta <= -10:
            body = (
                f"The thigh is dramatically constricted — fabric is at maximum stretch across the leg. Render "
                f"clear muscle contour through the cloth, horizontal tension lines wrapping the thigh, and side "
                f"seams under heavy load. Keep it plausible: too small, not torn."
            )
        elif delta <= -6:
            body = (
                f"The thigh is heavily strained. Render pronounced horizontal tension lines across the thigh, "
                f"with the muscle contour reading clearly through the fabric."
            )
        elif delta <= -3:
            body = (
                f"The thigh is clearly tight. Render faint horizontal tension lines across the front of the "
                f"thigh and the leg contour starting to read through the cloth."
            )
        elif delta < -1:
            body = (
                f"The thigh is snug. The fabric follows the muscle with light contact and no visible stretch "
                f"lines."
            )
        elif delta <= 1:
            body = (
                f"The thigh is true to size — straight, clean drape from hip to knee with no clinging and no "
                f"excess."
            )
        elif delta <= 3:
            body = (
                f"The thigh is relaxed. The fabric falls in a clean straight-leg line with a small air gap "
                f"around the thigh."
            )
        elif delta <= 6:
            body = (
                f"The thigh is loose. The fabric sits visibly off the leg with soft vertical folds along the "
                f"outer thigh, reading as a wide-leg drape."
            )
        else:
            body = (
                f"The thigh is heavily oversized — the leg silhouette is wide and palazzo-leaning regardless "
                f"of the intended cut, with deep vertical folds from hip to knee."
            )

    elif dim == "inseam":
        if delta <= -20:
            body = (
                f"The leg length is at the shorts/Bermuda range relative to a full-length pant. Render the hem "
                f"above the knee or at mid-thigh — if the garment is intended as shorts, this reads as the "
                f"intended length; otherwise it reads as far too short."
            )
        elif delta <= -10:
            body = (
                f"The hem sits at or near the knee. Render the lower leg fully exposed below the hem; this "
                f"reads as a Bermuda or knee-length cut on this wearer."
            )
        elif delta <= -6:
            body = (
                f"The hem is well above the ankle, sitting around the mid-calf. Render a clear length of bare "
                f"lower leg between hem and shoe."
            )
        elif delta <= -3:
            body = (
                f"The hem is cropped — sitting at the lower calf or upper ankle, with the ankle clearly "
                f"exposed above the shoe."
            )
        elif delta < -1:
            body = (
                f"The hem rides slightly above the ankle bone with a small visible band of bare ankle."
            )
        elif delta <= 1:
            body = (
                f"The hem lands at the intended landmark on the foot — clean break at the shoe with no "
                f"excess pooling."
            )
        elif delta <= 3:
            body = (
                f"The hem stacks slightly on the shoe with a small fold at the ankle."
            )
        elif delta <= 6:
            body = (
                f"The hem pools heavily at the ankle with multiple folds breaking over the shoe."
            )
        else:
            body = (
                f"The hem drags on the ground beyond the shoe, with the excess fabric pooling and folding "
                f"around the foot."
            )
