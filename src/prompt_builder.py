...
- A small negative chest delta on a Bodycon/Slim cut is intentional and reads sleek and tailored.
...
2. Explicitly instruct the image model to preserve every visible design detail from the garment reference image it will receive separately: print/logo/color blocking, collar shape, sleeve type, fabric weight, seams, cuffs, and texture.
3. Go through EACH of the 4 deltas (length, chest, shoulder, arm) using the band that the actual numeric delta falls into above. Be concrete and physical. Do not blur band boundaries — a -4 must read as "clearly tight with visible tension lines", a -10 must read as "heavily strained bodycon", a -20 must read as "fabric at maximum stretch, clearly the wrong size", and so on.
